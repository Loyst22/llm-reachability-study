...
app.layout = html.Div([
    html.H2("3D Metric Heatmap Viewer with Z Trends & Slicing"),
...
def update_heatmap(z_index, slice_info, equal_spacing, group_depths, selected_metric):
    ...
    if use_grouped:
        z_slice = metric_data['grouped'][z_index]
        current_x_labels = grouped_x_labels_str
    else:
        z_slice = metric_data['original'][z_index]
...
def update_slice_heatmap(slice_info, group_depths, selected_metric):
    if not slice_info:
        return go.Figure()
    ...
    use_grouped = 'group' in group_depths
    current_metric_data = metric_data['grouped'] if use_grouped else metric_data['original']
